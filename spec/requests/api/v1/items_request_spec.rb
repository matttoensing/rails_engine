...
RSpec.describe 'items api' do
  ...
  it 'happy path, fetch a page of items which would contain no data' do
    ...
    get '/api/v1/items', params: { page: 2_000_000 }
  ...
  it 'happy path, fetch all items if per page is really big' do
    ...
    get '/api/v1/items', params: { per_page: 2_000_000 }
  ...
  it 'can update an existing item with a merchant id included in params(happy path with partial data)' do
    ...
    patch "/api/v1/items/#{item.id}", headers: headers,
                                      params: JSON.generate(item: item_params, merchant_id: merchant.id)
  ...
  it 'will return 400 status code if merchant id is bad and not update an existing item(edge case, bad merchant id returns 400 or 404)' do
    ...
    patch "/api/v1/items/#{item.id}", headers: headers,
                                      params: JSON.generate(merchant_id: '9999999999', item: item_params)
  ...
  it 'edge case, string id returns 404' do
    ...
    patch '/api/v1/items/string-stringy-string', headers: headers,
