...
RSpec.describe 'merchants with most revenue api' do
  ...
  it 'happy path, top two merchants by revenue' do
    ...
    item1 = create(:item, unit_price: 99.99, merchant: merchant1)
    item2 = create(:item, unit_price: 19.99, merchant: merchant2)
    item3 = create(:item, unit_price: 29.99, merchant: merchant3)
    customer1 = create(:customer)
    customer2 = create(:customer)
    invoice1 = create(:invoice, customer: customer1, merchant: merchant1, status: 'shipped')
    invoice2 = create(:invoice, customer: customer1, merchant: merchant2, status: 'shipped')
    invoice3 = create(:invoice, customer: customer2, merchant: merchant2, status: 'shipped')
    invoice4 = create(:invoice, customer: customer2, merchant: merchant3, status: 'shipped')
    transaction1 = create(:transaction, invoice: invoice1, result: 'success')
    transaction2 = create(:transaction, invoice: invoice2, result: 'success')
    transaction3 = create(:transaction, invoice: invoice3, result: 'success')
    transaction4 = create(:transaction, invoice: invoice4, result: 'success')
    invoice_item1 = create(:invoice_item, unit_price: 99.99, quantity: 4, item: item1, invoice: invoice1)
    invoice_item2 = create(:invoice_item, unit_price: 19.99, quantity: 2, item: item2, invoice: invoice2)
    invoice_item3 = create(:invoice_item, unit_price: 49.99, quantity: 2, item: item2, invoice: invoice3)
    invoice_item4 = create(:invoice_item, unit_price: 19.99, quantity: 4, item: item3, invoice: invoice4)
  ...
  it 'happy path, all merchants if quantity is too big' do
    get '/api/v1/revenue/merchants', params: { quantity: 100_000 }
